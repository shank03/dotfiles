...
-- blank instead of '~' on lines past end-of-buffer
...
-- treesitter folding; foldexpr returns 0 when buffer has no parser, so safe globally
vim.o.foldmethod = 'expr'
vim.o.foldexpr = 'v:lua.vim.treesitter.foldexpr()'
vim.o.foldlevelstart = 99 -- open all folds on file open; fold on demand
...
require('lazy').setup({
  ...
  -- TODO: uncomment carbonfox if needed
  --
  ...
  { -- Highlight, edit, and navigate code
    ...
    -- `main` is the maintained branch for Neovim 0.11+/0.12; `master` is frozen and
    -- its query predicates crash on 0.12 (match captures became arrays of nodes).
    branch = 'main',
    build = ':TSUpdate',
    ...
    -- NOTE: Treesitter indent is experimental on `main`, so it is intentionally not
    -- enabled here; buffers fall back to the default indent behavior.
  },
